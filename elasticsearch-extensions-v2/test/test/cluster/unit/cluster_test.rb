...
class ElasticsearchV2::Extensions::TestClusterTest < Test::Unit::TestCase
  ...
  context "The Test::Cluster" do
    context "module" do
      should "delegate the methods to the class" do
        ...
        ElasticsearchV2::Extensions::Test::Cluster.start foo: 'bar'
        ElasticsearchV2::Extensions::Test::Cluster.stop foo: 'bar'
        ElasticsearchV2::Extensions::Test::Cluster.running? foo: 'bar'
        ElasticsearchV2::Extensions::Test::Cluster.wait_for_green foo: 'bar'
      end
    end

    context "class" do
      setup do
        ElasticsearchV2::Extensions::Test::Cluster::Cluster.any_instance.stubs(:__default_network_host).returns('_local_')

        @subject = ElasticsearchV2::Extensions::Test::Cluster::Cluster.new(number_of_nodes: 1)
      ...
      context "when stopping a cluster" do
        setup do
          @subject = ElasticsearchV2::Extensions::Test::Cluster::Cluster.new
        end
      ...
      context "when determining a version" do
        setup do
          @subject = ElasticsearchV2::Extensions::Test::Cluster::Cluster.new command: '/foo/bar/bin/elasticsearch'
        end
        ...
        should "return version from arguments" do
          cluster = ElasticsearchV2::Extensions::Test::Cluster::Cluster.new command: '/foo/bar/bin/elasticsearch', version: '5.2'
