...
Use the Backup gem's DSL to configure the backup:
    ...
    require 'elasticsearch_v2/extensions/backup'
...
See more information in the [`Backup::Database::ElasticsearchV2`](lib/extensions/backup.rb)
class documentation.
...
a `reindex` method is added to the client:
    ...
    require 'elasticsearch_v2/extensions/reindex'

    client = ElasticsearchV2::Client.new
    target_client = ElasticsearchV2::Client.new url: 'http://localhost:9250', log: true
...
You can also use the `Reindex` class directly:
    ...
    reindex = ElasticsearchV2::Extensions::Reindex.new \
                source: { index: 'test', client: client },
                target: { index: 'test-copy' }

    reindex.perform

See more information in the [`ElasticsearchV2::Extensions::Reindex::Reindex`](lib/extensions/reindex.rb)
class documentation.
...
Stop this cluster:
    ...
    ElasticsearchV2::Extensions::Test::Cluster.stop port: 9350
...
You can control the cluster configuration with environment variables as well:
    ...
    ruby -r elasticsearch -e "require 'elasticsearch_v2/extensions/test/cluster'; ElasticsearchV2::Extensions::Test::Cluster.start"
...
Let's define a simple profiling test in a `profiling_test.rb` file:
    ...
    require 'elasticsearch_v2/extensions/test/profiling'
