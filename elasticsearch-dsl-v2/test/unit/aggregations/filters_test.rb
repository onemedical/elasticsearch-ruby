    ...
    module Aggregations
      class FiltersTest < ::Test::Unit::TestCase
        ...
        context "Filters agg" do
          subject { ElasticsearchV2::DSL::Search::Aggregations::Filters.new }
          ...
          should "define filters" do
            subject = ElasticsearchV2::DSL::Search::Aggregations::Filters.new filters: { foo: 'bar' }
            assert_equal({ filters: { filters: { foo: 'bar' } } }, subject.to_hash)
          end

          should "define filters with a block" do
            subject = ElasticsearchV2::DSL::Search::Aggregations::Filters.new do
              filters foo: 'bar'
            end
            assert_equal({ filters: { filters: { foo: 'bar' } } }, subject.to_hash)
          end
