...
class ElasticsearchV2::Transport::ClientTest < Test::Unit::TestCase
  ...
  context "Client" do
    setup do
      ElasticsearchV2::Transport::Client::DEFAULT_TRANSPORT_CLASS.any_instance.stubs(:__build_connections)
      @client = ElasticsearchV2::Transport::Client.new
    end

    should "be aliased as ElasticsearchV2::Client" do
      assert_nothing_raised do
        assert_instance_of(ElasticsearchV2::Transport::Client, ElasticsearchV2::Client.new)
      end
    end

    should "have default transport" do
      assert_instance_of ElasticsearchV2::Transport::Client::DEFAULT_TRANSPORT_CLASS, @client.transport
    end

    should "instantiate custom transport class" do
      client = ElasticsearchV2::Transport::Client.new :transport_class => DummyTransport
      assert_instance_of DummyTransport, client.transport
    end

    should "take custom transport instance" do
      client = ElasticsearchV2::Transport::Client.new :transport => DummyTransport.new
      assert_instance_of DummyTransport, client.transport
    end
    ...
    should "send GET request as POST with the send_get_body_as option" do
      ...
      client = ElasticsearchV2::Transport::Client.new :transport => transport, :send_get_body_as => 'POST'
    ...
    should "pass options to the transport" do
      client = ElasticsearchV2::Transport::Client.new :transport_options => { :foo => 'bar' }
      assert_equal 'bar', client.transport.options[:transport_options][:foo]
    end

    should "merge request_timeout to the transport options" do
      client = ElasticsearchV2::Transport::Client.new :request_timeout => 120
    ...
    context "when passed hosts" do
      should "have localhost by default" do
        c = ElasticsearchV2::Transport::Client.new
        assert_equal 'localhost', c.transport.hosts.first[:host]
      end

      should "take :hosts, :host, :url or :urls" do
        c1 = ElasticsearchV2::Transport::Client.new :hosts => ['foobar']
        c2 = ElasticsearchV2::Transport::Client.new :host  => 'foobar'
        c3 = ElasticsearchV2::Transport::Client.new :url   => 'foobar'
        c4 = ElasticsearchV2::Transport::Client.new :urls  => 'foo,bar'
    ...
    context "when the URL is set in the environment variable" do
      setup    { ENV['ELASTICSEARCH_V2_URL'] = 'foobar' }
      teardown { ENV.delete('ELASTICSEARCH_V2_URL')     }
      ...
      should "use multiple hosts" do
        ENV['ELASTICSEARCH_V2_URL'] = 'foo, bar'
        c = ElasticsearchV2::Transport::Client.new
    ...
    context "detecting adapter for Faraday" do
      setup do
        ElasticsearchV2::Transport::Client::DEFAULT_TRANSPORT_CLASS.any_instance.unstub(:__build_connections)
      ...
      should "use the adapter from arguments" do
        c = ElasticsearchV2::Transport::Client.new :adapter => :typhoeus
        handlers = c.transport.connections.all.first.connection.builder.handlers
    ...
    context "configuring Faraday" do
      ...
      should "apply faraday adapter" do
        c = ElasticsearchV2::Transport::Client.new do |faraday|
    ...
    context "when passed options" do
      ...
      should "transfer selected host parts from URL into the 'http' options" do
        c = ElasticsearchV2::Transport::Client.new :url => 'https://U:P@node1:8080'
