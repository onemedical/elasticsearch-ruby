...
without any configuration:
    ...
    # => #<ElasticsearchV2::Transport::Transport::Response:0x007fc5d506ce38 @status=200, @body={ ... } >
...
To connect to a specific Elasticsearch host:
    ...
    ElasticsearchV2::Client.new host: 'search.myserver.com'

To connect to a host with specific port:

    ElasticsearchV2::Client.new host: 'myhost:8080'

To connect to multiple hosts:

    ElasticsearchV2::Client.new hosts: ['myhost1', 'myhost2']

Instead of Strings, you can pass host information as an array of Hashes:

    ElasticsearchV2::Client.new hosts: [ { host: 'myhost1', port: 8080 }, { host: 'myhost2', port: 8080 } ]
...
Common URL parts -- scheme, HTTP authentication credentials, URL prefixes, etc -- are handled automatically:
    ...
    ElasticsearchV2::Client.new url: 'https://username:password@api.server.org:4430/search'

You can pass multiple URLs separated by a comma:

    ElasticsearchV2::Client.new urls: 'http://localhost:9200,http://localhost:9201'

Another way to configure the URL(s) is to export the `ELASTICSEARCH_V2_URL` variable.
...
You can pass the authentication credentials, scheme and port in the host configuration hash:
    ...
    ElasticsearchV2::Client.new hosts: [
...
... or simply use the common URL format:
    ...
    ElasticsearchV2::Client.new url: 'https://username:password@example.com:9200'
...
set the `log` argument:
    ...
    ElasticsearchV2::Client.new log: true

To trace requests and responses in the _Curl_ format, set the `trace` argument:

    ElasticsearchV2::Client.new trace: true
...
Or, you can use a custom {::Logger} instance:
    ...
    ElasticsearchV2::Client.new logger: Logger.new(STDERR)
...
You can pass the client any conforming logger implementation:
    ...
    client = ElasticsearchV2::Client.new logger: log
...
To increase the timeout, you can use the `request_timeout` parameter:
    ...
    ElasticsearchV2::Client.new request_timeout: 5*60
...
collection on initialization and reloading:
    ...
    ElasticsearchV2::Client.new hosts: ['localhost:9200', 'localhost:9201'], randomize_hosts: true
...
on a different host:
    ...
    ElasticsearchV2::Client.new hosts: ['localhost:9200', 'localhost:9201'], retry_on_failure: true

You can specify how many times should the client retry the request before it raises an exception
(the default is 3 times):

    ElasticsearchV2::Client.new hosts: ['localhost:9200', 'localhost:9201'], retry_on_failure: 5
...
on every 10,000th request:
    ...
    ElasticsearchV2::Client.new hosts: ['localhost:9200', 'localhost:9201'], reload_connections: true

You can pass a specific number of requests after which the reloading should be performed:

    ElasticsearchV2::Client.new hosts: ['localhost:9200', 'localhost:9201'], reload_connections: 1_000

To reload connections on failures, use:

    ElasticsearchV2::Client.new hosts: ['localhost:9200', 'localhost:9201'], reload_on_failure: true

The reloading will timeout if not finished under 1 second by default. To change the setting:

    ElasticsearchV2::Client.new hosts: ['localhost:9200', 'localhost:9201'], sniffer_timeout: 3
...
**NOTE:** When using reloading hosts ("sniffing") together with authentication, just pass the scheme,
    ...
    ElasticsearchV2::Client.new host: 'localhost:9200',
...
{ElasticsearchV2::Transport::Transport::Connections::Selector::RoundRobin} strategy.
...
Only when these would be unavailable, the strategy will use the other nodes:
    ...
    class RackIdSelector
      include ElasticsearchV2::Transport::Transport::Connections::Selector::Base
    ...
    ElasticsearchV2::Client.new hosts: ['x1.search.org', 'x2.search.org'], selector_class: RackIdSelector
...
To use a specific adapter for _Faraday_, pass it as the `adapter` argument:
    ...
    client = ElasticsearchV2::Client.new adapter: :net_http_persistent
...
constructor, use the `transport_options` key:
    ...
    client = ElasticsearchV2::Client.new transport_options: {
...
To configure the _Faraday_ instance directly, use a block:
    ...
    client = ElasticsearchV2::Client.new(host: 'localhost', port: '9200') do |f|
      f.response :logger
      f.adapter  :typhoeus
    end
...
for example sign the requests for the [AWS Elasticsearch service](https://aws.amazon.com/elasticsearch-service/):
    ...
    client = ElasticsearchV2::Client.new url: 'https://search-my-cluster-abc123....es.amazonaws.com' do |f|
...
as the `transport` argument:
    ...
    transport = ElasticsearchV2::Transport::Transport::HTTP::Faraday.new \
      hosts: [ { host: 'localhost', port: '9200' } ],
      &transport_configuration

    # Pass the transport to the client
    #
    client = ElasticsearchV2::Client.new transport: transport
...
Instead of passing the transport to the constructor, you can inject it at run time:
    ...
    faraday_client = ElasticsearchV2::Transport::Transport::HTTP::Faraday.new \
...
You can also use a bundled [_Curb_](https://rubygems.org/gems/curb) based transport implementation:
    ...
    require 'curb'
    require 'elasticsearch_v2/transport/transport/http/curb'

    client = ElasticsearchV2::Client.new transport_class: ElasticsearchV2::Transport::Transport::HTTP::Curb
...
(in this case, as an inline block):
    ...
    transport = ElasticsearchV2::Transport::Transport::HTTP::Curb.new \
      hosts: [ { host: 'localhost', port: '9200' } ],
...
{ElasticsearchV2::Transport::Transport::Base} module, implementing the required contract,
...
{ElasticsearchV2::Transport::Transport::Serializer::Base} module, implementing the required contract,
...
The highest-level exception is {ElasticsearchV2::Transport::Transport::Error}
and will be raised for any generic client *or* server errors.

{ElasticsearchV2::Transport::Transport::ServerError} will be raised for server errors only.

As an example for response-specific errors, a `404` response status will raise
an {ElasticsearchV2::Transport::Transport::Errors::NotFound} exception.

Finally, {ElasticsearchV2::Transport::Transport::SnifferTimeoutError} will be raised
...
* {ElasticsearchV2::Transport::Client} is composed of {ElasticsearchV2::Transport::Transport}

* {ElasticsearchV2::Transport::Transport} is composed of {ElasticsearchV2::Transport::Transport::Connections},
...
* The {ElasticsearchV2::Transport::Transport::Serializer::Base} implementations handle converting data for Elasticsearch
  (eg. to JSON). You can implement your own serializer.

* {ElasticsearchV2::Transport::Transport::Sniffer} allows to discover nodes in the cluster and use them as connections.

* {ElasticsearchV2::Transport::Transport::Connections::Collection} is composed of
  {ElasticsearchV2::Transport::Transport::Connections::Connection} instances and a selector instance.

* {ElasticsearchV2::Transport::Transport::Connections::Connection} contains the connection attributes such as hostname and port,
  as well as the concrete persistent "session" connected to a specific node.

* The {ElasticsearchV2::Transport::Transport::Connections::Selector::Base} implementations allow to choose connections
