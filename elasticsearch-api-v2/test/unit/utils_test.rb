...
module ElasticsearchV2
  module Test
    class UtilsTest < ::Test::Unit::TestCase
      include ElasticsearchV2::API::Utils
      ...
      context "Utils" do
        ...
        context "__bulkify" do
          ...
          should "serialize array of hashes" do
            result = ElasticsearchV2::API::Utils.__bulkify [
          ...
          should "serialize arrays of strings" do
            result = ElasticsearchV2::API::Utils.__bulkify ['{"foo":"bar"}','{"moo":"bam"}']
            assert_equal <<-PAYLOAD.gsub(/^\s+/, ''), result
              {"foo":"bar"}
              {"moo":"bam"}
            PAYLOAD
          ...
          should "serialize arrays of header/data pairs" do
            result = ElasticsearchV2::API::Utils.__bulkify [{:foo => "bar"},{:moo => "bam"},{:foo => "baz"}]
            assert_equal <<-PAYLOAD.gsub(/^\s+/, ''), result
              {"foo":"bar"}
              {"moo":"bam"}
              {"foo":"baz"}
          ...
          should "not modify the original payload with the data option" do
            ...
            result = ElasticsearchV2::API::Utils.__bulkify original
        ...
        context "__validate_and_extract_params" do
          teardown do
            ElasticsearchV2::API.settings.clear
          end
