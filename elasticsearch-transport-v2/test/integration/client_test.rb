...
class ElasticsearchV2::Transport::ClientIntegrationTest < ElasticsearchV2::Test::IntegrationTestCase
  startup do
    ElasticsearchV2::Extensions::Test::Cluster.start(nodes: 2) if ENV['SERVER'] and not ElasticsearchV2::Extensions::Test::Cluster.running?
  end

  shutdown do
    ElasticsearchV2::Extensions::Test::Cluster.stop if ENV['SERVER'] and ElasticsearchV2::Extensions::Test::Cluster.running?
  end
  ...
  context "Elasticsearch client" do
    ...
    context "when reloading connections" do
      should "keep existing connections" do
        ...
        client = ElasticsearchV2::Transport::Client.new host: "localhost:#{@port}", adapter: :patron, logger: @logger
    ...
    context "with Faraday adapters" do
      should "set the adapter with a block" do
        ...
        client = ElasticsearchV2::Transport::Client.new url: "localhost:#{@port}" do |f|
      ...
      should "automatically use the Patron client when loaded" do
        ...
        require 'patron'
        client = ElasticsearchV2::Transport::Client.new host: "localhost:#{@port}"
