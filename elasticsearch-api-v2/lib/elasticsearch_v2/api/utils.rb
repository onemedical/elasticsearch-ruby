module ElasticsearchV2
  module API
    ...
    module Utils
      ...
      # in a single item.
      #
      #     ElasticsearchV2::API::Utils.__bulkify [
      ...
      def __bulkify(payload)
        ...
        when payload.any? { |d| d.is_a?(Hash) && d.values.first.is_a?(Hash) && operations.include?(d.keys.first.to_s) && (d.values.first[:data] || d.values.first['data']) }
          payload = payload.
            ...
            map { |item| ElasticsearchV2::API.serializer.dump(item) }
        ...
        # Header/Data pairs
          ...
          payload = payload.map { |item| ElasticsearchV2::API.serializer.dump(item) }
          payload << ''
        end
      ...
      #   ElasticsearchV2::API.settings[:skip_parameter_validation] = true
      ...
      def __validate_and_extract_params(arguments, params=[], options={})
        if options[:skip_parameter_validation] || ElasticsearchV2::API.settings[:skip_parameter_validation]
          arguments
        else
      ...
      def __report_unsupported_parameters(arguments, params=[])
        ...
        unsupported_params.each do |param|
          ...
          explanation = if param.is_a?(Hash)
            ...
            ". This parameter is not supported in the version you're using: #{ElasticsearchV2::API::VERSION}, and will be removed in the next release."
          end
      ...
      def __report_unsupported_method(name)
        ...
        message += ". This method is not supported in the version you're using: #{ElasticsearchV2::API::VERSION}, and will be removed in the next release."
